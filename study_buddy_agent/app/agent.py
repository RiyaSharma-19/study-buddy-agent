...
def answer_gate(ctx: Context, node_input: Any):
    ...
    # Resolve flashcards list from node_input (handles Pydantic model or dict)
    ...
    # If resumed and answer is submitted
    ...
    # First execution: request input from the user
    if not flashcards:
        raise ValueError("No flashcards generated by the Content Agent.")
...
tutor_agent = Agent(
    ...
    model=shared_model,
    instruction=(
        "You are the Tutor Agent. You will receive a JSON input containing a 'flashcard' "
        "(with correct question and answer) and a 'user_answer'. "
        "Evaluate the user's submitted answer against the correct answer in the flashcard.\n"
        "Classify the mistake type as exactly one of:\n"
        "- 'forgot': if the user didn't know the answer, left it blank, or couldn't recall it.\n"
        "- 'misunderstood': if the user has a misconception or wrong understanding of the concept.\n"
        "- 'careless error': if the user's answer is correct or has a minor typo, slip, or formatting mismatch but gets the core concept right.\n"
        "Provide a tailored explanation based on this classification to help the user understand and learn. "
