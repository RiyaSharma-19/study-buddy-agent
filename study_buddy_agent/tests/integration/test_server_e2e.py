...
def test_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    session_url = f"{BASE_URL}/apps/study_buddy_agent/users/{user_id}/sessions"
    ...
    # Workflow responds with either text content OR a functionCall (e.g. answer_gate
    # interrupt requesting user input) -- both are valid non-error responses.
    has_valid_content = has_text_content or any(
        event.get("content") is not None
        or event.get("longRunningToolIds")
        for event in events
    )
    assert has_valid_content, "Expected at least one non-error event from stream"
